...
int main(){
  freopen("../../../resources/2/4000/24511.txt", "r", stdin);
  ...
  deque<int> v_q;
  deque<bool> v_isQue;
  for(int i = 0 ; i<v_qs_size;i++){
    bool isQ;
    cin >> isQ;
    v_isQue.push_back(isQ);
  }
  for(int i = 0 ;i<v_qs_size;i++){
    int v_value;
    cin >> v_value;
    if(v_isQue[i] == 0){
      v_q.push_back(v_value);
    }
  }
  ...
  // for(int i =0;i<v_qs_size;i++){
  //   cout << v_qs[i].first << ' ' << v_qs[i].second << '\n';
  // }
...
1 초 (추가 시간 없음)	1024 MB (추가 메모리 없음)	5749	2092	1743	35.191%
문제
...
1 3 5
출처
Camp > ICPC Sinchon Algorithm Camp > 2022 ICPC Sinchon Winter Algorithm Camp Contest > 초급 C번
